...
export default function MinuOstudPage() {
      ...
      <section className={isSmallScreen ? 'w-full' : 'w-3/4 space-y-6'}>
        ...
        {visibleOrders.length === 0 ? (
          ...
          visibleOrders.map(order => {
            ...
            const delivered = order.status === 'delivered' || order.status === 'confirmed';
...
<span className="text-m font-bold">
  {order.seller?.first_name} {order.seller?.surname}
    ...
    <button
      ...
      className="w-full sm:w-auto px-4 py-2 mt-2 bg-none border text-black rounded-full hover:bg-white transition"
    >
      Saada sõnum
    </button>
  </div>

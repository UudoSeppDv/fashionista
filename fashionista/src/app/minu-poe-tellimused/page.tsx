...
interface Order {
  ...
  public_users?: UserInfo;  // lisame siia
}


const PAGE_SIZE = 3;

export default function MinuPoeTellimusedPage() {
        ...
        .select(`
          *,
          product:product_id (
           ...
           public_users:user_id (first_name, surname, avatar_url)
        `)
        .eq('seller_id', userId)
...
<div className="flex flex-col sm:flex-row items-center justify-between pt-4 gap-4">
  ...
  <div className="flex items-center gap-3">
    {order.public_users?.avatar_url ? (
      <div className="relative w-14 h-14 rounded-full overflow-hidden">
        <Image
          src={order.public_users.avatar_url}
          alt="Profiilipilt"
          fill
      ...
      <div className="w-14 h-14 rounded-full bg-pink-300 text-white flex items-center justify-center text-xl font-bold">
        {order.public_users?.first_name?.charAt(0)}
        {order.public_users?.surname?.charAt(0)}
      </div>
    )}
    <span className="text-m font-bold">
      {order.public_users?.first_name} {order.public_users?.surname}
    </span>
  </div>
